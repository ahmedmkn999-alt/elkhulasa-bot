...
const VIDEO_BOT_TOKEN   = "8777919219:AAEc_mZ9I-jdfPTcnRUf9VKL9cWg3Nh3Rc4";
...
async function saveUser(userId, username, firstName) {
  await db.ref(`users/${userId}`).set({
    ...
    firstName:  firstName || "",
    status:     "new",       // new = لم يجرب بعد
    trialStart: null,
    joinedAt:   Date.now(),
    banned:     false,
    waitingCode: false,      // ينتظر إدخال كود اشتراك
  });
}
...
function mainKeyboard(user) {
  const now = Date.now();
  const inTrial = user.status === "trial" && (now - user.trialStart) < TRIAL_DURATION_MS;
  const isActive = user.status === "active" && user.subscriptionEnd > now;

  const keyboard = [];

  if (inTrial || isActive) {
    keyboard.push([{ text: "📚 استعرض المواد", callback_data: "show_subjects" }]);
  }

  if (user.status === "new" || (!inTrial && !isActive && user.status !== "active")) {
    keyboard.push([{ text: "🎁 جرب مجاناً — 30 دقيقة", callback_data: "start_trial" }]);
  }

  keyboard.push([{ text: "🔑 اشتراك — أدخل كودك", callback_data: "enter_code" }]);
...
function subjectsKeyboard() {
  return {
    inline_keyboard: [
      ...
      [{ text: "🔙 الرئيسية", callback_data: "back_main" }],
    ],
  };
}
...
function getStatusLine(user) {
  const now = Date.now();
  if (user.banned) return "🚫 <b>محظور</b>";
  if (user.status === "trial" && (now - user.trialStart) < TRIAL_DURATION_MS)
    return `⏱ <b>تجربة مجانية</b> — متبقي: ${trialLeft(user)}`;
  if (user.status === "active" && user.subscriptionEnd > now)
    return `✅ <b>مشترك</b> حتى ${new Date(user.subscriptionEnd).toLocaleDateString("ar-EG")}`;
  if (user.status === "trial" && (now - user.trialStart) >= TRIAL_DURATION_MS)
    return "⌛ <b>انتهت التجربة المجانية</b>";
  return "🔒 <b>غير مشترك</b>";
}
...
async function handleClient(update) {
  ...
  if (msg) {
    ...
    // ======= حالة انتظار إدخال الكود =======
    if (user.waitingCode) {
      const code = text.trim();

      // تحقق إن الكود أرقام 6 خانات
      if (!/^\d{6}$/.test(code)) {
        await send(CLIENT_BOT_TOKEN, chatId,
          "❌ الكود يجب أن يكون <b>6 أرقام</b> بالضبط.\nأعد الإرسال أو اضغط /start للرجوع."
        );
        return;
      }

      // ابحث عن الكود في Firebase
      const codeSnap = await db.ref(`codes/${code}`).get();
      if (!codeSnap.exists()) {
        await send(CLIENT_BOT_TOKEN, chatId,
          `❌ <b>الكود غير صحيح!</b>\n\nتأكد من الكود أو تواصل مع الإدارة:\n${ADMIN_USERNAME}`
        );
        return;
      }
      ...
      if (codeData.used) {
        await send(CLIENT_BOT_TOKEN, chatId,
          `❌ <b>هذا الكود تم استخدامه من قبل!</b>\n\nتواصل مع الإدارة للحصول على كود جديد:\n${ADMIN_USERNAME}`
        );
        return;
      }

      // ✅ الكود صحيح — فعّل الاشتراك
      ...
      await send(CLIENT_BOT_TOKEN, chatId,
        ...
        `📅 ينتهي في: <b>${new Date(subEnd).toLocaleDateString("ar-EG")}</b>\n\n` +
        `👇 اختر المادة وابدأ مذاكرتك!`,
  ...
  if (cb) {
    ...
    // الرئيسية
    if (data === "back_main") {
      user = await getUser(userId); // reload
      const statusLine = getStatusLine(user);
      await send(CLIENT_BOT_TOKEN, chatId,
        `🎓 <b>الخُلاصة</b>\n\n${statusLine}`,
        { reply_markup: mainKeyboard(user) }
    ...
    if (data === "start_trial") {
      ...
      if (user.status === "trial" || user.trialStart) {
        await send(CLIENT_BOT_TOKEN, chatId,
          `⚠️ <b>لقد استخدمت التجربة المجانية من قبل!</b>\n\n` +
          `للاشتراك تواصل مع الإدارة:\n${ADMIN_USERNAME}`
        );
        return;
      }
      // أول مرة يجرب
      await db.ref(`users/${userId}`).update({ status: "trial", trialStart: Date.now() });
      user = await getUser(userId);
      await send(CLIENT_BOT_TOKEN, chatId,
        `🎁 <b>تم تفعيل تجربتك المجانية!</b>\n\n` +
        `⏱ لديك <b>30 دقيقة</b> لتجربة البوت كاملاً.\n\n` +
        `👇 اختر المادة وابدأ الآن!`,
        { reply_markup: subjectsKeyboard() }
      );
      return;
    }
    ...
    // ======= زر "أدخل كودك" =======
    if (data === "enter_code") {
      await db.ref(`users/${userId}`).update({ waitingCode: true });
      await send(CLIENT_BOT_TOKEN, chatId,
        `🔑 <b>أدخل كود الاشتراك</b>\n\n` +
        `أرسل الكود المكون من <b>6 أرقام</b> الذي حصلت عليه من الإدارة.\n\n` +
        `للحصول على كود: ${ADMIN_USERNAME}`
      );
      return;
    }
    ...
    if (data.startsWith("sub_")) {
      ...
      if (!hasAccess(user)) {
        await send(CLIENT_BOT_TOKEN, chatId,
          ...
          { reply_markup: { inline_keyboard: [[{ text: "🔑 أدخل كود الاشتراك", callback_data: "enter_code" }]] } }
        );
        return;
      }

      const header = user.status === "trial"
        ? `⏱ متبقي من التجربة: <b>${trialLeft(user)}</b>\n\n`
        : "";
...
async function handleAdmin(update) {
  ...
  if (text === "/start") {
    await send(ADMIN_BOT_TOKEN, chatId,
      ...
      `<b>🔑 الكودات:</b>\n` +
      `/gencode 30 — توليد كود لمدة 30 يوم\n` +
      `/codes — عرض الكودات النشطة\n\n` +
      `<b>المستخدمون:</b>\n` +
      `/ban @user — حظر\n` +
      `/unban @user — رفع حظر\n` +
  ...
  // ---- /gencode [أيام] — توليد كود اشتراك ----
  if (text.startsWith("/gencode")) {
    const parts = text.split(" ");
    const days  = parseInt(parts[1]) || 30;

    // توليد كود 6 أرقام عشوائي فريد
    let code;
    let exists = true;
    while (exists) {
  ...
  if (text === "/codes") {
    ...
    if (!snap.exists()) { await send(ADMIN_BOT_TOKEN, chatId, "لا يوجد كودات."); return; }
    const codes = snap.val();
    const active = Object.entries(codes).filter(([,v]) => !v.used);
    const used   = Object.entries(codes).filter(([,v]) => v.used);
    let txt = `🔑 <b>الكودات</b>\n\n`;
    if (active.length) {
      txt += `✅ <b>نشطة (${active.length}):</b>\n`;
      active.forEach(([code, v]) => {
        txt += `<code>${code}</code> — ${v.days} يوم\n`;
      });
    }
    if (used.length) {
      txt += `\n❌ <b>مستخدمة (${used.length}):</b>\n`;
      used.slice(-5).forEach(([code, v]) => {
        txt += `<code>${code}</code> — @${v.username||"?"}\n`;
      });
    }
  ...
  if (text === "/stats") {
    ...
    const codesCount = codesSnap.exists() ? Object.values(codesSnap.val()).filter(c => !c.used).length : 0;
    await send(ADMIN_BOT_TOKEN, chatId,
  ...
  if (text.startsWith("/resettrial ")) {
    ...
    let uid;
    snap.forEach(c => {
      updates[`users/${c.key}/status`]     = "new";
      updates[`users/${c.key}/trialStart`] = null;
      uid = c.val().userId;
    });
    await db.ref().update(updates);
    if (uid) await send(CLIENT_BOT_TOKEN, uid, `🎁 تم تجديد إمكانية تجربتك المجانية! اضغط /start 🚀`);
    await send(ADMIN_BOT_TOKEN, chatId, `✅ تم إعادة ضبط تجربة @${uname}`);
    return;
  }
...
module.exports = async (req, res) => {
  ...
  if (req.method === "GET") {
    if (urlPath === "/" || urlPath === "/dashboard" || urlPath === "") {
      try {
        const html = fs.readFileSync(path_module.join(__dirname, "../dashboard.html"), "utf8");
        res.setHeader("Content-Type", "text/html; charset=utf-8");
        return res.status(200).send(html);
      } catch {
        return res.status(200).send("الخُلاصة ✅");
      }
    }
    return res.status(200).send("الخُلاصة ✅");
  }
  ...
  try {
    ...
    if      (urlPath.includes("/api/client")) await handleClient(update);
    else if (urlPath.includes("/api/admin"))  await handleAdmin(update);
    else if (urlPath.includes("/api/video"))  await handleVideoAdmin(update);
